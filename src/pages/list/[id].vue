		...
		<template v-else>
			<div class="flex justify-between items-center mb-6">
				...
				<div class="flex space-x-2">
					<button v-if="!list.isCompleted" @click="finishShopping"
						class="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700 transition shadow-md flex items-center space-x-2">
						<CheckIcon class="w-5 h-5" />
						<span>Finalizar Compras</span>
					</button>
					<NuxtLink to="/"
						class="bg-gray-100 px-6 py-2 rounded-full hover:bg-gray-200 transition flex items-center space-x-2">
				...
				<div class="p-4">
					<div class="space-y-4">
						<div v-for="(item, index) in list.items" :key="index"
							...
							<input type="checkbox" v-model="item.purchased" :disabled="list.isCompleted"
							...
							<div v-if="!list.isCompleted" class="flex items-center space-x-2">
								<div class="flex items-center space-x-2 mt-1">
									<input v-model="item.quantity" type="number" min="1" :disabled="list.isCompleted"
								...
								<div class="relative">
									...
									<input type="number" v-model="item.price" step="0.01" placeholder="0,00"
										class="pl-8 w-28 rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
										@input="updateSubtotal" />
								</div>
								<span class="text-sm text-gray-600">
									Total: R$ {{ (item.price * item.quantity || 0).toFixed(2) }}
								</span>
								<button @click="removeItem(index)" class="text-red-600 hover:text-red-800 p-1">
									<TrashIcon class="w-5 h-5" />
								</button>
							</div>
							<div v-else class="text-sm text-gray-600">
								Preço: R$ {{ item.price?.toFixed(2) }} | Total: R$ {{ (item.price * item.quantity ||
									0).toFixed(2) }}
							</div>
						</div>
					</div>
								...
								<form @submit.prevent="addNewItem" class="mt-4">
									<div class="space-y-4">
										<div>
											...
											<input v-model="newItem.name" type="text" placeholder="Ex: Arroz"
												class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
												required />
										</div>
										<div>
											<label class="block text-sm font-medium text-gray-700">Quantidade</label>
											<input v-model="newItem.quantity" type="number" min="1" placeholder="1"
												class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500"
												required />
										</div>
									</div>
